...
public class DSAQuestions {
    ...
    public int findLargest(int[] arr) {
        ...
        for (int num : arr) { // Traverse all elements
            if (num > max) max = num; // Update max if a larger element is found
        }
        return max;
    }
    ...
    public boolean isSorted(int[] arr) {
        for (int i = 1; i < arr.length; i++) {
            if (arr[i] < arr[i - 1]) return false; // If any pair is out of order, return false
        }
    ...
    public int findMissingNumber(int[] arr, int n) {
        ...
        int actualSum = 0;
        for (int num : arr) actualSum += num; // Sum all elements in the array
    ...
    public Set<Integer> findDuplicates(int[] arr) {
        ...
        for (int num : arr) {
            if (seen.contains(num)) duplicates.add(num); // If seen before, it's a duplicate
            else seen.add(num); // Otherwise, add to seen set
        }
        return duplicates;
    }
    ...
    public void moveZeroesToEnd(int[] arr) {
        int insertPos = 0;
        for (int num : arr) {
            if (num != 0) arr[insertPos++] = num; // Shift non-zero elements forward
        }
        while (insertPos < arr.length) arr[insertPos++] = 0; // Fill remaining positions with zeros
    }

    // 10. Find the intersection of two arrays
    public int[] intersect(int[] nums1, int[] nums2) {
        Map<Integer, Integer> map = new HashMap<>();
        for (int num : nums1) map.put(num, map.getOrDefault(num, 0) + 1); // Count each element in nums1
    ...
    public int firstUniqChar(String s) {
        ...
        for (char c : s.toCharArray()) count.put(c, count.getOrDefault(c, 0) + 1); // Count each character
        for (int i = 0; i < s.length(); i++) {
            if (count.get(s.charAt(i)) == 1) return i; // Return first unique character's index
        }
        return -1;
    }
    ...
    public int binarySearch(int[] arr, int target) {
        ...
        while (left <= right) {
            ...
            if (arr[mid] == target) return mid; // Return index if target is found
    ...
    public int findKthLargest(int[] nums, int k) {
        ...
        for (int num : nums) {
            minHeap.add(num);
            if (minHeap.size() > k) minHeap.poll(); // Remove smallest to keep k largest
        }
        return minHeap.peek();
    }
    ...
    public boolean isPalindrome(ListNode head) {
        ...
        if (fast != null) slow = slow.next; // Skip the middle element if odd length
        while (slow != null) {
    ...
    public int lengthOfLIS(int[] nums) {
        ...
        int maxLIS = 0;
        for (int length : dp) maxLIS = Math.max(maxLIS, length); // Find the maximum LIS in dp array
        return maxLIS;
    }
    ...
    public int findPeakElement(int[] nums) {
        ...
        while (left < right) {
            int mid = left + (right - left) / 2;
            if (nums[mid] > nums[mid + 1]) right = mid; // Descending side of peak
            else left = mid + 1; // Ascending side of peak
        }
        return left;
    }
